...
import './LeaderboardSection.css'; // Assuming this CSS file will be updated
...
const LeaderboardSection = ({ courseId, userId, token, refreshTrigger }) => {
    ...
    // Omitted commented-out useEffects for brevity, as they were not requested to be changed.
    ...
    const fetchLeaderboardData = async () => {
        try {
            ...
            const [leaderboardResponse, rankResponse, statsResponse] = await Promise.all([
                axios.get(
                    // NOTE: Hardcoded URL 'http://localhost:5000' is used, consider using an environment variable
    ...
    // Kept for potential future use or external consumption, although not used in the refactored render.
                ...
                <div className="leaderboard-content">
                    {leaderboard.length === 0 ? (
                        ...
                        <div className="leaderboard-list-container">
                            {/* Refactored Header Div */}
                            <div className="leaderboard-header">
                                <div className="header-cell rank-cell">Rank</div>
                                <div className="header-cell user-cell">User</div>
                                <div className="header-cell roll-cell">Roll Number</div>
                                <div className="header-cell department-cell">Department</div>
                                <div className="header-cell score-cell">Score</div>
                            </div>
                            
                            {/* Refactored Leaderboard Rows */}
                            <div className="leaderboard-body">
                                {leaderboard.map((entry, index) => (
                                    <div 
                                        key={`${entry.userId || entry.user?._id || 'user'}-${entry.rank || index}-${index}`}
                                        className={`leaderboard-row rank-${entry.rank} ${(entry.userId || entry.user?._id) === userId ? 'current-user' : ''}`}
                                    >
                                        <div className="rank-cell">
                                            <span className={`badge rank-badge rank-badge-${entry.rank}`}>
                                                {entry.rank}
                                            </span>
                                        </div>
                                        <div className="user-cell user-name">{entry.name || entry.user?.name || 'Anonymous'}</div>
                                        <div className="roll-cell">{entry.rollNumber || entry.user?.rollNumber || 'N/A'}</div>
                                        <div className="department-cell">{entry.department || entry.user?.department || 'N/A'}</div>
                                        <div className="score-cell total-score">{entry.overallScore || 0}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
