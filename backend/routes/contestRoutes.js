...
// Fetch upcoming contests (for users)
router.get("/upcoming", async (req, res) => {
  try {
    ...
    const contests = await Contest.find({
      startTime: { $gt: now },
    }).populate("problems").sort({ startTime: 1 });

    console.log("Found upcoming contests:", contests.length);
    res.json(contests);
  } catch (error) {
    console.error("Error in upcoming contests endpoint:", error);
    res.status(500).json({ message: "Server Error" });
  }
});
