...
// Get user's global ranking (based on problem submissions)
const getUserGlobalRank = async (req, res) => {
  try {
    const userId = req.user.id;
    console.log('Getting global rank for user:', userId);
    ...
    submissionData.forEach(sub => {
      submissionMap[sub._id.toString()] = {
        totalScore: sub.totalScore,
        totalSolved: sub.totalSolved
      };
    });

    // Combine user data with submission data
    const leaderboardData = usersInCollege.map(user => {
      const submissionInfo = submissionMap[user._id.toString()] || { totalScore: 0, totalSolved: 0 };
      return {
        _id: user._id,
        name: user.name,
        email: user.email,
        ...
        totalSolved: submissionInfo.totalSolved
      };
    });
    ...
    const userIndex = leaderboardData.findIndex(user => user._id.toString() === userId.toString());
    const userRank = userIndex !== -1 ? userIndex + 1 : leaderboardData.length + 1;
    const totalUsers = leaderboardData.length;

    // Calculate percentile
    const percentile = totalUsers > 0 ? Math.round(((totalUsers - userRank + 1) / totalUsers) * 100) : 0;

    // Get user's current stats
    const userStats = leaderboardData[userIndex] || { totalScore: 0, totalSolved: 0 };
    ...
    console.error("Error fetching user global rank:", err);
    res.status(500).json({ message: "Error fetching user global rank", error: err.message });
  }
};
